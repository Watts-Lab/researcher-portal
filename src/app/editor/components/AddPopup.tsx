...
export default function AddPopup({
  ...
  function handleSave(saveType: string) {
    ...
    if (saveType === "addStage") {
      ...
      const inputs: {name: string, duration: number, elements: any[]} = {
        name: nameValue,
        duration: parseInt(durationValue),
        elements: [],
      };
      ...
      for (const key in elementValues) {
        ...
        if (value != "") {
          inputs[key as keyof typeof inputs] = value;
        }
      }
  ...
  const handleInputChange = (property: string, newValue: any) => {
  ...
  const handleURLChange = (newValue: string) =>
    handleInputChange("url", newValue);
