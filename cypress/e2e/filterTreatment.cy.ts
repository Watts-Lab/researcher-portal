describe('timeline filter treatment', () => {
    beforeEach(() => {
        ...
        // assumes first stage has been added by default treatment
        ...
        cy.get('[data-cy="stage-0"]').contains("TestTemplateA").should("be.visible");
    ...
    it('should dynamically populate filter options based on treatment file', () => {
        cy.get('[data-cy="stages-dropdown"] select').within(() => {
            ...
            cy.get('option').eq(1).should('have.text', 'TestTemplateA');
